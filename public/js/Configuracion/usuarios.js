...
function llenar_cmb_empresa() {
    $.ajax({
        type: "GET",
        dataType: "json",
        url: base_url + "/Configuracion/Ctrl_usuarios/llenar_cmb_empresa",
    }).done( function(data) {
        $("#cmb_empresa").html('');

        var opciones_apr = "<option value=\"\">Seleccione una Empresa</option>";
        
        for (var i = 0; i < data.length; i++) {
            opciones_apr += "<option value=\"" + data[i].id + "\">" + data[i].nombre + "</option>";
        }

        $("#cmb_empresa").append(opciones_apr);
    }).fail(function(error){
