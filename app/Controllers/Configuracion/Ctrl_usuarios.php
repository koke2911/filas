...
class Ctrl_usuarios extends BaseController {
  ...
  public function datatable_usuarios() {
    $this->validar_sesion();
    // echo $this->sesión->id_empresa_ses;
    echo $this->usuarios->datatable_usuarios($this->db, $this->sesión->id_empresa_ses,$this->sesión->es_admin);
  }

  public function llenar_cmb_empresa() {
    $this->validar_sesion();
    $id_empresa=$this->sesión->id_empresa_ses;
    $es_admin=$this->sesión->es_admin;

    if($es_admin!=0){
      $datos_apr = $this->empresas->select("id")
                           ->select("nombre as nombre")
                           ->findAll();
    }else{
      $datos_apr = $this->empresas->select("id")
                           ->select("nombre as nombre")->where("id",$id_empresa)
                           ->findAll();
    }
